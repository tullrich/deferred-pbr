...
GLuint utility_load_texture_dds(const char* filepath) {
  ...
  for (std::size_t face = 0; face < Texture.faces(); face++) {
    for (std::size_t level = 0; level < Texture.levels(); level++) {
      ...
      printf("\tFace: %lu Mip: %lu Extents: <%i, %i, %i> Format: %#010x Type: %#010x External %#010x\n"
        , face, level, level_extent.x, level_extent.y, level_extent.z, format.Internal, format.Type, format.External);
